    ...
    if "error" not in habitat_data:
        ...
        for period in key_periods:
            ...
            if real_period in rentability:
                print(
                    f"   • {period.replace('_nominal', '')}: {rentability[period]:.2f}% nominal, {rentability[real_period]:.2f}% real"
                )
    else:
...
def demo_afp_comparison(data_manager):
    ...
    comparison = data_manager.compare_afp_rentability(
        ["Habitat", "Integra", "Prima"], 0
    )
...
def demo_fund_analysis(data_manager):
    ...
    if "fund_analysis" in fund_analysis:
        for fund_key, fund_info in fund_analysis["fund_analysis"].items():
            ...
            if (
                "average_rentability" in fund_info
                and "averages" in fund_info["average_rentability"]
            ):
                ...
                if recent_keys:
                    ...
                    print(
                        f"   • Rentabilidad promedio: {avg_data[nominal_key]:.2f}% nominal, {avg_data[real_key]:.2f}% real"
                    )
